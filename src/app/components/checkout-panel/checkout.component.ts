...
export class CheckoutPanelComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.name =
      this.commonService.user.firstName +
      ' ' +
      this.commonService.user.lastName;
    ...
    this.totalAmount =
      this.commonService.booking.bookedDays *
      this.commonService.booking.costPerDay;
  }
  completeBooking() {
    if (this.commonService.user.wallet > this.totalAmount) {
      ...
      this.walletData.wallet =
        this.commonService.user.wallet - this.totalAmount;
